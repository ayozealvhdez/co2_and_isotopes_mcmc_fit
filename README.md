...
When reproducing the paper figures, first run the scripts in `scripts/fit/` for the five series fitted in the paper, using the adopted model configurations: CO2, δ13CO2 and Δ14CO2 at IZO, and CO2 and δ13CO2 at MLO. Then generate the paper-specific figures using the scripts in `scripts/additional_paper_figures/`. Note that some paper figures (Fig. 2 and Figs. B1-B3) do not have dedicated scripts in `scripts/additional_paper_figures/`; they are generated by the corresponding reusable scripts in `scripts/fit/` and `scripts/residual_analysis/` when running them for the appropriate configurations.

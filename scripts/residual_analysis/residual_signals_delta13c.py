"""
Compare the Lomb-Scargle periodograms of the residuals obtained from two previous MCMC runs of delta13CO2 in the 'fmin' to 'fmax' yr^-1 frequency range.
...
run_1_label = "IZO"
site_acronym_1 = "IZO"
recompute_monthly_series_1 = True
...
run_2_label = "MLO"
site_acronym_2 = "MLO"
recompute_monthly_series_2 = True
...
data_tag_1 = "monthly" if recompute_monthly_series_1 else "discrete"
data_tag_2 = "monthly" if recompute_monthly_series_2 else "discrete"
noise_data_tag = "monthly" if noise_recompute_monthly_series else "discrete"

results_dir_1 = run_results_directory(project_root, "delta13c", site_acronym_1, data_tag_1, include_slow_harmonics_1, base_period_slow_harmonics_1, slow_harmonics_1, polynomial_degree=polynomial_degree_1)
results_dir_2 = run_results_directory(project_root, "delta13c", site_acronym_2, data_tag_2, include_slow_harmonics_2, base_period_slow_harmonics_2, slow_harmonics_2, polynomial_degree=polynomial_degree_2)
noise_results_dir = run_results_directory(project_root, "delta13c", noise_site_acronym, noise_data_tag, noise_include_slow_harmonics, noise_base_period_slow_harmonics, noise_slow_harmonics, polynomial_degree=noise_polynomial_degree)

plot_dir = comparison_directory(project_root, "delta13c_residual_periodograms")
...
output_path = os.path.join(plot_dir, f"delta13c_residual_periodograms_comparison_{run_1_label}_vs_{run_2_label}.png")
...
for pk, fit in zip(peaks_idx_1, fits_1):
    ...
    if np.isfinite(fit["sigma"]) and fit["sigma"] > 0:
        ...
        print(f"f = {f:.4f} 1/yr | sigma = {fit['sigma']:.4f} | Power = {p:.3g} | FAP_white = {fap_white:.3f}% | FAP_red = {fap_red:.3f}%")
    else:
        f = frequency_1[pk]
        print(f"f = {f:.4f} 1/yr | sigma = nan | Power = {p:.3g} | FAP_white = not computed | FAP_red = not computed")
print("-------------------------------------------------------")
...
for fit in fits_1:
    ax.vlines(fit["mu"], bar_ymin, bar_ymax, colors=color_1, lw=1.2, linestyle="-", zorder=3)

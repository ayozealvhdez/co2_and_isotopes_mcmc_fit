"""
Plot the fitted delta14C series, the posterior median model, the 68% confidence band, the posterior median p(t) + l(t) component with its 68% confidence band, and the residuals from a previous MCMC run performed with 'fit_mcmc_delta14c.py'.
...
- Upper panel: Observed delta14C series alongside the posterior median model and the posterior median p(t) + l(t) component, with shaded bands indicating 68% CL derived from Monte Carlo with joint posterior vectors. Lower panel: residuals of the fit.
"""

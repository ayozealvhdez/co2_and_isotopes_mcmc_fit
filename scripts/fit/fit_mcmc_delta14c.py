...
results_and_plots/delta14c/<site_acronym.lower()>/<data_tag>/<model_tag>/
...
from functions.delta14c_data_filtering import filter_valid_delta14C_values, filter_delta14C_analysis_flag, filter_delta14C_dates_by_month_range
from functions.delta14c_data_time_axis import compute_monthly_means_delta14C, center_month_midpoint, to_decimal_year
...
data_directory = os.path.join(project_root, "data", "delta14c")
...
print(f"Step 2: Filter the data: remove invalid values, and keep only data with valid QC flags and between {start_month} and {end_month}")
dates, values, uncertainties, flags, analytical_flags = filter_valid_delta14C_values(dates, values, uncertainties, flags, analytical_flags)
dates, values, uncertainties, flags, analytical_flags = filter_delta14C_analysis_flag(dates, values, uncertainties, flags, analytical_flags)
dates, values, uncertainties, flags, analytical_flags = filter_delta14C_dates_by_month_range(dates, values, uncertainties, flags, analytical_flags, start_month, end_month)
print("-------------------------------------------------------")
...
if recompute_monthly_series:
    ...
    dates, delta14c, stds, nvalues = compute_monthly_means_delta14C(dates, values, uncertainties)

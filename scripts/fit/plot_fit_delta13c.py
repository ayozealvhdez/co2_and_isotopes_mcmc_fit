...
- Upper panel: Observed delta13C series alongside the best-fit model from the MCMC fitting, with shaded band indicating 68% CL derived from Monte Carlo
...
# ---------------- GRID CONFIGURATION -------------------
# -------------------------------------------------------
start_decimal_year_for_grid = 1992.0
...
print("Step 2: Build the grid of timestamps used to calculate the model curve and uncertainty band")
...
ax1.set_ylabel(r"$\delta^{13}$C-CO$_2$ (‰)", size=20)
...
ax2.set_ylabel(r"$\delta^{13}$C-CO$_2$ - $f(t)$ (‰)", size=20)

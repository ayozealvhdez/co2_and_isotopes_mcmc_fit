"""
Plot the fitted CO2 series, the posterior median model, the 68% confidence band, the posterior median p(t) + l(t) component with its 68% confidence band, and the residuals from a previous MCMC run performed with 'fit_mcmc_co2.py'.

The script reads:
- 'samples_for_MC.txt', containing posterior samples drawn from the MCMC chain during the execution of 'fit_mcmc_co2.py'
...
- Upper panel: Observed CO2 series alongside the posterior median model and the posterior median p(t) + l(t) component, with shaded bands indicating 68% CL derived from Monte Carlo with joint posterior vectors. Lower panel: residuals of the fit.
"""
...
ax1.set_ylabel("CO$_2$ mole fraction (ppm)", size=20)
...
ax2.set_ylabel("CO$_2$ - $f(t)$ (ppm)", size=20)
